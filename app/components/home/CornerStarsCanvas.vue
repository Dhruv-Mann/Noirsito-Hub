...
// Quantum Constellation Implosion Burst & Permutation Swap
function triggerConstellationImplosion(hoveredSlot: number) {
  isCollapsing = true
  const centerX = width / 2
  const centerY = height / 2

  // 1. Pull all 4 stars rapidly inward towards center focal point
  stars.forEach(star => {
  ...
  setTimeout(() => {
    ...
    // Permute slot indices (rotate clockwise + shift colors)
    stars.forEach(star => {
...
function render() {
  ...
  if (!props.isStarted) {
    // 1. Draw laser-thin geometric constellation lines connecting 4 stars across screen
    ctx.beginPath()
    ...
    // 2. Render implosion spark particles
    ...
    // 3. Render 4 Lissajous stars with spring Lerp physics
    stars.forEach(star => {
