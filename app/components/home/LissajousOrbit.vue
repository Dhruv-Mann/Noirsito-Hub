...
const render = () => {
  ...
  // When props.isStarted is true (user clicks), force delta to Math.PI / 2
  // which locks the curve into the perfect symmetric central cross before expanding!
  let delta: number
  if (props.isStarted) {
    delta = Math.PI / 2
  } else {
...
// Immediately render the cross state as soon as props.isStarted triggers
